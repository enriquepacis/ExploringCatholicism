...
#+begin_src zsh :results output
  ...
  git commit -m "Adds some videos by Dr. David Anders (change in doctrines)."
  git pull
  git push
#+end_src

#+RESULTS:
: [main d4af6c1] Adds some videos by Dr. David Anders (change in doctrines).
:  7 files changed, 1873 insertions(+), 1595 deletions(-)
: Already up to date.
