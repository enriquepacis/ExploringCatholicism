...
#+begin_src zsh :results output
  ...
  git commit -m "Adds a video by mother angelica on fallacies in modern culture."
  git pull
  git push
#+end_src

#+RESULTS:
: [main 2673c75] Adds a video by mother angelica on God's plan in every state of life.
:  3 files changed, 1369 insertions(+), 1341 deletions(-)
: Already up to date.
